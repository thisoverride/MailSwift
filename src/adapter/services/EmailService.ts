...
import { EmailMessagePayload } from '../../@types/Service';
...
export default class EmailService {
  ...
  private _loadTemplate(templateName: string, context: Record<string, any>): string {
    const filePath: string = path.join(__dirname, '..', '..', '..', 'templates', `${templateName}.hbs`);
  ...
  public async sendEmail(options: EmailMessagePayload): Promise<HttpResponse> {
    try {
      let html: string;
      let subject: string;
